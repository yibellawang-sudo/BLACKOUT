"use client";
import { useState } from "react";
import { MEMES } from "@/lib/memes";

interface IdeaModalProps {
  onClose: () => void;
  onIdeaSubmitted: (data: { text: string; meme: string }) => void;
}

export default function IdeaModal({ onClose, onIdeaSubmitted }: IdeaModalProps) {
  const [text, setText] = useState("");
  const [meme, setMeme] = useState<string>("");
  const [submitted, setSubmitted] = useState(false);

  const selectedMeme = MEMES.find(m => m.id === meme);

  const handleSubmit = () => {
    if (!text || !meme) return;
    onIdeaSubmitted({ text, meme });
    setSubmitted(true);
  };

  return (
    <div style={{
      position: 'fixed',
      inset: 0,
      zIndex: 50,
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      backgroundColor: 'rgba(0,0,0,0.8)',
      padding: '1rem'
    }}>
      <div style={{
        position: 'relative',
        backgroundColor: '#1a1d2e',
        borderRadius: '1rem',
        maxWidth: '32rem',
        width: '100%',
        padding: '2rem',
        border: '1px solid rgba(255,255,255,0.1)'
      }}>
        <button 
          onClick={onClose}
          style={{
            position: 'absolute',
            top: '1rem',
            right: '1rem',
            width: '2rem',
            height: '2rem',
            borderRadius: '50%',
            backgroundColor: 'rgba(255,255,255,0.1)',
            border: 'none',
            color: '#fff',
            cursor: 'pointer',
            fontSize: '1.5rem'
          }}
        >
          ×
        </button>

        {!submitted ? (
          <>
            <h2 style={{ color: '#fff', marginBottom: '1.5rem', fontSize: '1.5rem' }}>
              Start typing your idea...
            </h2>

            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', color: '#a3a3a3', marginBottom: '0.5rem' }}>
                Idea Description
              </label>
              <textarea
                style={{
                  width: '100%',
                  padding: '0.75rem',
                  backgroundColor: 'rgba(255,255,255,0.05)',
                  border: '1px solid rgba(255,255,255,0.1)',
                  borderRadius: '0.5rem',
                  color: '#fff',
                  minHeight: '5rem',
                  fontFamily: 'inherit'
                }}
                placeholder="Type your idea here..."
                value={text}
                onChange={(e) => setText(e.target.value)}
              />
            </div>

            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', color: '#a3a3a3', marginBottom: '0.5rem' }}>
                Choose a Meme
              </label>
              <select
                style={{
                  width: '100%',
                  padding: '0.75rem',
                  backgroundColor: 'rgba(255,255,255,0.05)',
                  border: '1px solid rgba(255,255,255,0.1)',
                  borderRadius: '0.5rem',
                  color: '#fff',
                  fontFamily: 'inherit',
                  cursor: 'pointer'
                }}
                value={meme}
                onChange={(e) => setMeme(e.target.value)}
              >
                <option value="">Select a meme...</option>
                {MEMES.map((m) => (
                  <option key={m.id} value={m.id}>
                    {m.label}
                  </option>
                ))}
              </select>

              {/*Meme Preview*/}
              {selectedMeme && (
                <div style={{
                  marginTop: '1rem',
                  padding: '1rem',
                  backgroundColor: 'rgba(255,255,255,0.03)',
                  borderRadius: '0.5rem',
                  border: '1px solid rgba(252, 211, 77, 0.2)'
                }}>
                <img 
                    src={selectedMeme.image} 
                    alt={selectedMeme.label}
                    style={{
                        width: '100%',
                        maxWidth: '250px',
                        height: 'auto',
                        borderRadius: '0.5rem',
                        display: 'block'
                    }}
                />
                </div>
              )}
            </div>

            <button
              style={{
                width: '100%',
                padding: '0.75rem',
                backgroundColor: 'rgba(252, 211, 77, 0.15)',
                border: '1px solid rgba(252, 211, 77, 0.3)',
                borderRadius: '0.5rem',
                color: '#fff',
                cursor: !text || !meme ? 'not-allowed' : 'pointer',
                opacity: !text || !meme ? 0.4 : 1
              }}
              onClick={handleSubmit}
              disabled={!text || !meme}
            >
              Save Idea
            </button>
          </>
        ) : (
          <div style={{ textAlign: 'center', padding: '2rem 0' }}>
            <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>✓</div>
            <h3 style={{ color: '#fff', marginBottom: '0.5rem' }}>Idea Submitted</h3>
            <p style={{ color: '#737373', marginBottom: '1.5rem' }}>
              Your idea has been added to the power grid.
            </p>
            <button
              style={{
                width: '100%',
                padding: '0.75rem',
                backgroundColor: 'rgba(255,255,255,0.05)',
                border: '1px solid rgba(255,255,255,0.1)',
                borderRadius: '0.5rem',
                color: '#a3a3a3',
                cursor: 'pointe r'
              }}
              onClick={() => {
                setSubmitted(false);
                setText("");
                setMeme("");
              }}
            >
              Submit Another
            </button>
          </div>
        )}
      </div>
    </div>
  );
}